    ...
    def create_activity_from_strava(self, activity: dict, get_streams: bool = True):
        ...
        try:
            ...
            if r.status_code == 200:
                heartrate = self.filter_response_by_key(r.json(), 'heartrate', [None])
        except Exception as e:
            logger.debug(f"No heartrate data for activity {activity_id}: {e}")

        # Fetch cadence if available
        cadence = None
        try:
            r = self.client.get(
                self.base_url + f"/activities/{activity_id}/streams",
            ...
            if r.status_code == 200:
                cadence = self.filter_response_by_key(r.json(), 'cadence', [None])
        except Exception as e:
            logger.debug(f"No cadence data for activity {activity_id}: {e}")
...
class Activity:
    ...
    def as_gpx(self) -> gpxpy.gpx.GPX:
        ...
        # Check if we have heartrate or cadence data
        has_hr = self.heartrate and len(self.heartrate) > 0
        has_cad = self.cadence and len(self.cadence) > 0

        # Create points:
        for i, (time, lat, long, alt, vel) in enumerate(zip(
            self.time, self.lat, self.long, self.altitude, self.velocity
        )):
            pt = gpxpy.gpx.GPXTrackPoint(lat, long, elevation=alt, time=time, speed=vel)

            # Add extensions if we have heartrate or cadence
            if has_hr or has_cad:
                hr_val = self.heartrate[i] if has_hr and i < len(self.heartrate) else None
    ...
    def _fix_gpxtpx_namespace(self, gpx_xml: str) -> str:
        """Add gpxtpx namespace prefix to extensions in GPX XML."""
        import re
        ...
        # Replace TrackPointExtension elements with prefixed version
        def add_prefix(match):
            inner = match.group(1)
            inner = re.sub(r'<TrackPointExtension>', '<gpxtpx:TrackPointExtension>', inner)
            inner = re.sub(r'</TrackPointExtension>', '</gpxtpx:TrackPointExtension>', inner)
            inner = re.sub(r'<hr>', '<gpxtpx:hr>', inner)
            inner = re.sub(r'</hr>', '</gpxtpx:hr>', inner)
            inner = re.sub(r'<cad>', '<gpxtpx:cad>', inner)
            inner = re.sub(r'</cad>', '</gpxtpx:cad>', inner)
            return f'<extensions>{inner}</extensions>'
        gpx_xml = re.sub(r'<extensions>(.*?)</extensions>', add_prefix, gpx_xml, flags=re.DOTALL)
        return gpx_xml

    def as_xml(self):
        """Export this activity's GPX (XML) representation as a string."""
        gpx_xml = self.as_gpx().to_xml()
        # Fix namespace prefixes for FitTrackee compatibility
        return self._fix_gpxtpx_namespace(gpx_xml)
